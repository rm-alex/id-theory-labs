Немного начал, попросил гпт нагенерить, но не проверял, пока что придётся отвлечься от этой ЛР

% OLS_estimation_zad11_zad12.m
% Выполнить в MATLAB: поместите zad11 и zad12 в рабочую область (workspace)
% Скрипт оценивает параметры линейной регрессии y = x1*theta1 + x2*theta2 + x3*theta3 + v
% Для каждого набора (zad11 и zad12) выводит оценки, стандартные ошибки, t-статистики,
% строит графики y и y_hat, график ошибки и даёт базовую диагностику (средняя ошибка, DW).
