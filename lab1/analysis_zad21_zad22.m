% Скрипт для сравнения линейной (H1) и квадратичной (H2) аппроксимаций
% Требование: в рабочей области есть zad21 и zad22 (каждый имеет поля .T и .V).
...
for k = 1:numel(datasets)
    ...
    % Оценки МНК (нормальное уравнение / backslash)
    beta1 = X1 \ V;
    beta2 = X2 \ V;
    ...
    % критерии: значимая F (p<palpha) --> H2 лучше; иначе предпочитаем прощею модель
    if pF < alpha && SSE2 < SSE1
        ...
        elseif AIC1 < AIC2 && adjR2_1 >= adjR2_2
            ...
            better = 'Результаты смешанные: используйте графики остатков и предметную интерпретацию.';
        end
    end
    fprintf('РЕКОМЕНДАЦИЯ: %s\n', better);

    % --- Построение графиков ---
    figure('Name', ['Аппроксимации и остатки — ' name], 'NumberTitle', 'off', 'Position',[100,100,900,600]);

    % 1) точки данных и аппроксимации
    subplot(2,2,1);
    hold on; box on;
    scatter(T, V, 40, 'filled');
    % гладкая линия для модели
    ...
    legend('эксперимент', 'линейная H1', 'квадратичная H2', 'Location','best');
    xlabel('T (°C)'); ylabel('V (s^{-1})');
    ...
    xlabel('T (°C)'); ylabel('остатки V - \^V (H1)');
    title('Остатки для H1 (линейная)'); box on;

    % 3) остатки H2
    subplot(2,2,3);
    plot(T, res2, 'o');
    hold on; plot([min(T), max(T)], [0,0], 'k--'); hold off;
    xlabel('T (°C)'); ylabel('остатки V - \^V (H2)');
    title('Остатки для H2 (квадратичная)'); box on;

    % 4) сравнение абсолютных ошибок / scatter residuals H1 vs H2
    subplot(2,2,4);
    scatter(res1, res2, 30, 'filled');
    xlabel('residuals H1'); ylabel('residuals H2');
    title('Сравнение остатков H1 vs H2'); grid on; box on;
    % диагональ
    hold on; lims = axis; plot(lims(1:2), lims(1:2), 'k--'); axis(lims); hold off;

    % --- Доп. диагностика: нормальность остатков (Shapiro-Wilk unavailable in base MATLAB)
    % можно показать гистограммы и Q-Q вручную
    figure('Name', ['Диагностика остатков — ' name], 'NumberTitle', 'off', 'Position',[150,150,800,400]);
    subplot(1,2,1);
    histogram(res1);
    title('Гистограмма остатков H1');
    xlabel('residual'); ylabel('count'); box on;
    subplot(1,2,2);
    histogram(res2);
    title('Гистограмма остатков H2');
    xlabel('residual'); ylabel('count'); box on;
end
